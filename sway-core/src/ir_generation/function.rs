...
impl FnCompiler {
    ...
    fn compile_lazy_op(
        ...
        if !self.current_block.is_terminated(context) {
            ...
            match ast_op {
                LazyOp::And => cond_builder
                    .conditional_branch(lhs_val, rhs_block, final_block, Some(lhs_val))
                    .add_metadatum(context, span_md_idx),
                LazyOp::Or => cond_builder
                    .conditional_branch(lhs_val, final_block, rhs_block, Some(lhs_val))
                    .add_metadatum(context, span_md_idx),
            };
        }
    ...
    fn compile_fn_call(
...

        {
            ...
            let state_idx_md_idx = match self_state_idx {
                Some(self_state_idx) => {
                    md_mgr.state_idx_to_md(context, self_state_idx.to_usize())
                }
                None => None,
            };
            Ok(self
    ...
    fn compile_const_decl(
        ...
        let const_expr_val = compile_constant_expression(context, md_mgr, self.module, None, &value)?;
        let local_name = self.lexical_map.insert(name.as_str().to_owned());
